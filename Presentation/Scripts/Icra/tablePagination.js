function initIcraTablePagination(tableId, pageSizeSelectId, paginationNavId, tableInfoId) {
    var currentPage = 1;
    ...
    function renderTable() {
        ...
        // İcra tablosuna özel arama sınıfı: .icra-column-search
        $(".icra-column-search").each(function () {
        ...
        // İcra tablosuna özel sayaç ID'leri
        $("#icraTotalRecordCount").text(totalRecords);
        $("#icraCurrentPageNum").text(currentPage);
        $("#icraTotalPagesNum").text(totalPages);
    ...
    // İcra tablosunda 6 veri sütunu bulunduğundan "İşlemler" sütununun indeksi 6'dır.
    $("#" + tableId + " thead tr:first-child th").on("click", function () {
        if ($(this).index() === 6) return;
        $("#" + tableId + " thead tr.search-row").toggle();
    });

    $(document).off("keyup change", ".icra-column-search")
        .on("keyup change", ".icra-column-search", function () {
            currentPage = 1;
            renderTable();
        });
